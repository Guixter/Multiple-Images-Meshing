function [idxbest, Cbest, sumDbest, Dbest] = KmeansSlic(X, k, m_sur_S, varargin)
%KMEANS K-means clustering.
%   
%   IDX = KMEANS_slic(X, K) partitions the points in the N-by-P data matrix X
...
distNames = {'sqeuclidean','cityblock','cosine','correlation','hamming','slic'};
    ...
    function cellout = loopBody(rep,S)
        ...
        switch start
            ...
            case 'cluster'
                ...
                [~, C] = kmeans_slic(Xsubset, k, m_sur_S, varargin{:}, 'start','sample', 'replicates',1);
            case 'numeric'
                C = CC(:,:,rep);
            case {'plus','kmeans++'}
                ...
                for ii = 2:k                    
                    sampleProbability = min(distfun(X,C(1:ii-1,:),distance, 0, 0, 0, m_sur_S),[],2);
        ...
        D = distfun(X, C, distance, 0, rep, reps, m_sur_S);
        [d, idx] = min(D, [], 2);
        ...
        try % catch empty cluster errors and move on to next rep
            ...
            nonempties = find(m>0);
            D(:,nonempties) = distfun(X, C(nonempties,:), distance, iter, rep, reps, m_sur_S);
            d = D((idx-1)*n + (1:n)');
        ...
        function converged = batchUpdate()
            ...
            while true
                ...
                [C(changed,:), m(changed)] = gcentroids(X, idx, changed, distance, m_sur_S);
                D(:,changed) = distfun(X, C(changed,:), distance, iter, rep, reps, m_sur_S);
                    ...
                    switch emptyact
                        ...
                        case 'singleton'
                            for i = empties
                                ...
                                m(i) = 1;
                                idx(lonely) = i;
                                D(:,i) = distfun(X, C(i,:), distance, iter, rep, reps, m_sur_S);
                                
                                % Update clusters from which points are taken
                                [C(from,:), m(from)] = gcentroids(X, idx, from, distance, m_sur_S);
                                D(:,from) = distfun(X, C(from,:), distance, iter, rep, reps, m_sur_S);
...
function D = distfun(X, C, dist, iter,rep, reps, m_sur_S)
...
switch dist
    ...
    case 'slic'
        for i = 1:nclusts
            dp = sqrt((X(:, 1) - C(i, 1)).^2 + (X(:, 2) - C(i, 2)).^2);
            da = sqrt((X(:, 3) - C(i, 3)).^2 + (X(:, 4) - C(i, 4)).^2 + (X(:, 5) - C(i, 5)).^2);
            D(:, i) = sqrt(da.^2 + (m_sur_S^2) * (dp.^2));
        end
end
end % function
...
function [centroids, counts] = gcentroids(X, index, clusts, dist, m_sur_S)
